...
func (c *Chain) GetContractAt(contractAddr common.Address, path string) *Contract {
	artifact, err := ReadArtifact(path)
	if err != nil {
		panic(err)
	}

	log.Printf("Binding contract at %s", contractAddr.Hex())

	contract := sdk.GetContract(contractAddr, artifact.Abi, c.clt)
	return &Contract{addr: contractAddr, clt: c.clt, kettleAddr: c.kettleAddr, Abi: artifact.Abi, contract: contract}
}
